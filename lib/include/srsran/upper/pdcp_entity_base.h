...
#include "srsran/upper/ip_reassemble.h"
...
// Default PDCP concatenating timer value (can be overridden via configuration)
#define DEFAULT_CONCATENATING_TIMER 5 // ms
...
{
public:
  pdcp_entity_base(task_sched_handle task_sched_, srslog::basic_logger& logger, srsran::timer_handler* timers_);
  ...
  void start_encrypted_pcap(srsran::pdcp_pcap* pcap_) { encrypted_pcap = pcap_; }
  void start_plaintext_pcap(srsran::pdcp_pcap* pcap_) { plaintext_pcap = pcap_; }
  void start_gw_pcap(srsran::gw_pcap* pcap_) { gw_pcap = pcap_; }
  ...
  virtual void set_last_cqi(srsran_cqi_type_t cqi_type, uint8_t cqi) = 0;
  virtual void set_last_mcs(uint32_t mcs_idx)                        = 0;
...
protected:
  ...
  srsran::pdcp_pcap* encrypted_pcap;
  srsran::pdcp_pcap* plaintext_pcap;
  ...
  pdcp_config_t cfg = {1,
                       ...
                       false,
                       srsran_rat_t::lte,
                       DEFAULT_CONCATENATING_TIMER};
  std::string   rb_name;
  uint32_t      concatenating_timer_ms = DEFAULT_CONCATENATING_TIMER;  // Configurable concatenating timer
  ...
  // Improved PDCP
  srsran::timer_handler::unique_timer concatenating_timer;
  byte_buffer_queue                   tx_sdu_queue;
  byte_buffer_queue                   failed_transmitted_queue;
  pdcp_utils::ip_pkt_t                rx_ip_packet{};
  uint32_t                            tbs = int(552 / 8) * 10;
  std::mutex                          mutex;
};

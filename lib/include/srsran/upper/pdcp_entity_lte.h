...
public:
  ...
  void encapsulate_sdu(unique_byte_buffer_t sdu, int sn = -1);
...
private:
  ...
  // Store the last CQI and MCS info
  srsran_cqi_type_t last_cqi_type  = SRSRAN_CQI_TYPE_WIDEBAND;
  uint8_t           last_cqi_value = 0;
  uint32_t          last_mcs_idx   = 0;

  bool           defense_running = false;
  const uint32_t fixed_sdu_size  = 1500;
};

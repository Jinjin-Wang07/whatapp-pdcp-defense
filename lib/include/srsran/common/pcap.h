...
// for saving encrypted and plaintext PDCP packet
#define PDCP_ENCRYPTED_DLT 161
#define PDCP_PLAIN_DLT 162
...
/* Context information for every user plan data (eNB) that will be logged */
typedef struct UP_Context_Info_s {
  unsigned short rnti;
  unsigned char  direction;
  unsigned short channelId; /* for SRB: 1=SRB1, 2=SRB2, 3=SRB1bis; for DRB: DRB ID */
...
#define UP_DATA_TYPE_Normal 0
#define UP_DATA_TYPE_Multicast 1
...
/* Write an individual UP (eNB) PDU (PCAP packet header + UP-context + UP-pdu */
int LTE_PCAP_UP_Write_PDU(FILE* fd, UP_Context_Info_t* context, const unsigned char* PDU, unsigned int length);

/* Write an individual PDCP (both encrypted and plaintext) PDU (PCAP packet header + PDCP) */

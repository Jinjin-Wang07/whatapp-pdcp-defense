...
int pdcp::add_bearer(uint32_t lcid, const pdcp_config_t& cfg)
{
  ...
  if (cfg.rat == srsran::srsran_rat_t::lte) {
    entity.reset(new pdcp_entity_lte{rlc, rrc, gw, task_sched, logger, lcid, timers});
    entity->start_encrypted_pcap(encrypted_pcap);
    entity->start_plaintext_pcap(plaintext_pcap);
    entity->start_gw_pcap(gw_pcap);
  } else if (cfg.rat == srsran::srsran_rat_t::nr) {
    entity.reset(new pdcp_entity_nr{rlc, rrc, gw, task_sched, logger, lcid, timers});
    entity->start_encrypted_pcap(encrypted_pcap);
    entity->start_plaintext_pcap(plaintext_pcap);
    entity->start_gw_pcap(gw_pcap);
  }
...
void pdcp::set_last_cqi(srsran_cqi_type_t cqi_type, uint8_t cqi)
{
  for (auto& lcid_it : pdcp_array) {
    lcid_it.second->set_last_cqi(cqi_type, cqi);
  }
}

void pdcp::set_last_mcs(uint32_t mcs_idx)
{
  for (auto& lcid_it : pdcp_array) {
    lcid_it.second->set_last_mcs(mcs_idx);
  }
}
} // namespace srsran

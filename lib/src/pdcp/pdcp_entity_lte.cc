...
pdcp_entity_lte::pdcp_entity_lte(srsue::rlc_interface_pdcp* rlc_,
  ...
  pdcp_entity_base(task_sched_, logger, timers_), rlc(rlc_), rrc(rrc_), gw(gw_)
{
  // Initial state
...
bool pdcp_entity_lte::configure(const pdcp_config_t& cnfg_)
{
  ...
  if (is_drb()) {
    if (not concatenating_timer.is_valid()) {
      logger.error("concatenating timer is no valid");
    }
    if (not concatenating_timer.is_running()) {
      // Get concatenating timer value from configuration
      concatenating_timer_ms = cfg.concatenating_timer_ms;
      concatenating_timer.set(concatenating_timer_ms, [this](uint32_t tid) { timer_expired(tid); });
      logger.info("concatenating_timer_ms == %u", concatenating_timer_ms);
      concatenating_timer.run();
    }
  }
  return true;
}
...
void pdcp_entity_lte::reset()
{
  ...
  if (concatenating_timer.is_running()) {
    concatenating_timer.stop();
  }

  failed_transmitted_queue.reset();
}
...
void pdcp_entity_lte::write_sdu(unique_byte_buffer_t sdu, int upper_sn)
{
  ...
  if (gw_pcap != nullptr && !is_srb()) {
    gw_pcap->write_dl_pdu(sdu->msg, sdu->N_bytes);
  }

  if (!is_srb()) {
    std::lock_guard<std::mutex> lock(mutex);
    uint32_t                    n_sdus = failed_transmitted_queue.get_n_sdus();
    logger.warning("%u sdus waiting for retransmission", n_sdus);
    for (uint32_t i = 0; i < n_sdus; i++) {
      unique_byte_buffer_t sdu_ = failed_transmitted_queue.read();
      if (not rlc->sdu_queue_is_full(lcid)) {
        encapsulate_sdu(std::move(sdu_));
      } else {
        failed_transmitted_queue.write(std::move(sdu_));
      }
    }

    tx_sdu_queue.try_write(std::move(sdu));

    return;
  } else {
    // srb
    encapsulate_sdu(std::move(sdu), upper_sn);
  }
}

void pdcp_entity_lte::encapsulate_sdu(unique_byte_buffer_t sdu, int upper_sn)
{
  ...
  if (!is_srb()) {
    if (plaintext_pcap != nullptr) {
      logger.info("Writng pcap... %u", sdu->N_bytes);
      plaintext_pcap->write_ul_pdu(0, lcid, tx_count, sdu->msg, sdu->N_bytes); // NOTE: UL/DL direction depends on the programme entity (UE/ENB)
    }
  }
  ...
  if (!rlc->rb_is_um(lcid) and is_drb()) {
    if (not store_sdu(used_sn, sdu)) {
      ...
      failed_transmitted_queue.write(std::move(sdu));
      return;
    }
  }
  ...
  if (!is_srb() && encrypted_pcap != nullptr) {
    encrypted_pcap->write_ul_pdu(0, lcid, tx_count, sdu->msg + 2, sdu->N_bytes - 2);
  }
...
void pdcp_entity_lte::handle_um_drb_pdu(srsran::unique_byte_buffer_t pdu)
{
  ...
  if (encrypted_pcap != nullptr) {
    encrypted_pcap->write_dl_pdu(0, lcid, sn, pdu->msg, pdu->N_bytes);
  }
  ...
  if (plaintext_pcap != nullptr) {
    plaintext_pcap->write_dl_pdu(0, lcid, sn, pdu->msg, pdu->N_bytes);
  }
...
void pdcp_entity_lte::handle_am_drb_pdu(srsran::unique_byte_buffer_t pdu)
{
  ...
  while (rx_ip_packet.append(pdu, sn) == 1) {
    if (gw_pcap != nullptr) {
      gw_pcap->write_ul_pdu(rx_ip_packet.data->msg, rx_ip_packet.data->N_bytes);
    }
    gw->write_pdu(lcid, std::move(rx_ip_packet.data));
    rx_ip_packet.reset();
  }
}

void pdcp_entity_lte::timer_expired(uint32_t timeout_id)
{
  if (concatenating_timer.is_valid() and concatenating_timer.id() == timeout_id) {
    // reset the timer
    // if (concatenating_timer.is_valid() && not concatenating_timer.is_running()) {
    //   logger.info("Reseting timer");
    //   concatenating_timer.set(CONCATENATING_TIMER, [this](uint32_t tid) { timer_expired(tid); });
    //   concatenating_timer.run();
    // }

    std::unique_lock<std::mutex> lock(mutex, std::try_to_lock);
    unique_byte_buffer_t         contents = make_byte_buffer();
    uint32_t                     room;
    uint8_t                      nof_pkt;
    uint32_t                     n_sdus = failed_transmitted_queue.get_n_sdus();

    if (!contents) {
      printf("Failed to initialize contents!\n");
      goto exit;
    }

    if (!lock.owns_lock()) { // failed in acquiring a lock
      goto exit;
    }
    ...
    if (!defense_running && tx_sdu_queue.get_n_sdus() > 0) {
      defense_running = true;
    }

    // iterate read all packets
    while (tx_sdu_queue.get_n_sdus() > 0) {
      uint32_t first_sz = tx_sdu_queue.size_tail_bytes();

      if (first_sz > fixed_sdu_size) {
        tx_sdu_queue.read(); // dropped
        printf("SDU size larger than fixed transmission size, not fully supported\n");
        break;
      }

      // sz < fixed_sdu_sz
      // read at most fixed_sdu_sz data from the queue
      if (first_sz + contents->N_bytes > fixed_sdu_size) {
        // run out of space stop reading
        break;
      }

      unique_byte_buffer_t sdu_seg_ = tx_sdu_queue.read();
      if (sdu_seg_->N_bytes + contents->N_bytes >= SRSRAN_MAX_BUFFER_SIZE_BYTES) {
        printf("PDCP Buffer overflow! SDU Drop\n");
        goto exit;
      }

      contents->append_bytes(sdu_seg_->msg, sdu_seg_->N_bytes);
    }

    if (contents->N_bytes == 0) { // No packet to be sent, send a dummy packet
      if (!is_srb() && defense_running) {
        unique_byte_buffer_t empty_sdu                     = make_byte_buffer();
        uint8_t              empty_payload[fixed_sdu_size] = {0};
        empty_sdu->append_bytes(empty_payload, sizeof(empty_payload));

        if (!rlc->sdu_queue_is_full(lcid)) {
          logger.info("No data available, sending an empty SDU.");
          logger.info("PDCP Sending...");        // used to check the delta time
          encapsulate_sdu(std::move(empty_sdu)); // Send empty packets
        } else {
          logger.warning("RLC queue is full, unable to send empty SDU.");
        }
        goto exit;
      } else {
        goto exit;
    ...
    if (contents->N_bytes > 0 && !is_srb()) {
      //  send the first sdu
      // when total sdu size < fixed_sdu_size
      if (contents->msg == nullptr) {
        printf("contents->msg is null!\n");
        goto exit;
      }

      if (contents->N_bytes <= fixed_sdu_size) {
        // padding and send the packet
        unique_byte_buffer_t sdu = make_byte_buffer();
        sdu->append_bytes(contents->msg, contents->N_bytes);

        if (!rlc->sdu_queue_is_full(lcid)) {
          uint32_t             padding_size = fixed_sdu_size - contents->N_bytes;
          unique_byte_buffer_t padding      = make_byte_buffer(padding_size, 0); // Pad 0
          sdu->append_bytes(padding->msg, padding->N_bytes);
          contents->N_bytes = 0;
          logger.info("PDCP Sending..."); // used to check the delta time
          encapsulate_sdu(std::move(sdu));
        } else {
          logger.warning("RLC queue is full, unable to send SDU.");
          failed_transmitted_queue.write(std::move(sdu));
        }
        goto exit;
      } else {
...
bool pdcp_entity_lte::store_sdu(uint32_t sn, const unique_byte_buffer_t& sdu)
{
  ...
  if (ret == false) {
    logger.warning(
        "Add SDU failed, sn=%d, FMS=%d, window=%d", sn, undelivered_sdus->get_fms(), maximum_allocated_sns_window);
  }
...
void pdcp_entity_lte::set_last_cqi(srsran_cqi_type_t cqi_type, uint8_t cqi)
{
  last_cqi_type  = cqi_type;
  last_cqi_value = cqi;
}
void pdcp_entity_lte::set_last_mcs(uint32_t mcs_idx)
{

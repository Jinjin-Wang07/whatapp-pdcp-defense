...
pdcp_entity_nr::pdcp_entity_nr(srsue::rlc_interface_pdcp* rlc_,
  ...
  pdcp_entity_base(task_sched_, logger, timers_),
  rlc(rlc_),
  rrc(rrc_),
  gw(gw_),
...
void pdcp_entity_nr::timer_expired(uint32_t timeout_id)
{
...
void pdcp_entity_nr::set_last_cqi(srsran_cqi_type_t cqi_type, uint8_t cqi)
{
  last_cqi_type  = cqi_type;
  last_cqi_value = cqi;
}
void pdcp_entity_nr::set_last_mcs(uint32_t mcs_idx)
{

...
uint32_t gw_pcap::open(const char* filename)
{
  ...
  pcap_file = DLT_PCAP_Open(UP_DLT, filename);
...
void gw_pcap::close()
{
  enable_write = false;
  if (pcap_file != nullptr) {
    fprintf(stdout, "Saving GW PCAP file (DLT 154) to %s\n", pcap_filename.c_str());
    DLT_PCAP_Close(pcap_file);
    pcap_file = nullptr;
  }
}

void gw_pcap::write_dl_pdu(uint8_t* pdu, uint32_t pdu_len_bytes)
{
  if (enable_write) {
...
void gw_pcap::write_dl_pdu_mch(uint8_t* pdu, uint32_t pdu_len_bytes)
{
  if (enable_write) {
    GW_Context_Info_t context;
    LTE_PCAP_GW_Write_PDU(pcap_file, &context, pdu, pdu_len_bytes);
...
void gw_pcap::write_ul_pdu(uint8_t* pdu, uint32_t pdu_len_bytes)
{
  if (enable_write) {
    GW_Context_Info_t context;
    LTE_PCAP_GW_Write_PDU(pcap_file, &context, pdu, pdu_len_bytes);

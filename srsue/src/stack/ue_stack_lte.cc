...
int ue_stack_lte::init(const stack_args_t& args_)
{
  ...
  if (args.pkt_trace.gw_pcap.enable) {
    if (gw_pcap.open(args.pkt_trace.gw_pcap.filename.c_str()) == SRSRAN_SUCCESS) {
      ...
      stack_logger.info("Open gw pcap file %s", args.pkt_trace.gw_pcap.filename.c_str());
    } else {
  ...
  if (!args.sa_mode) {
    pdcp.init(&rlc, &rrc, gw, task_sched.get_timer_handler());
  } else {
    pdcp.init(&rlc, &rrc, &sdap_pdcp, task_sched.get_timer_handler());
  ...
  pdcp_nr.init(&rlc_nr, &rrc_nr, gw, task_sched.get_timer_handler());
  rrc_nr.init(phy_nr,
              &mac_nr,
              &rlc_nr,
              &pdcp_nr,

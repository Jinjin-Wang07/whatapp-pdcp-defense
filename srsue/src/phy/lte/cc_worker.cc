...
bool cc_worker::encode_uplink(mac_interface_phy_lte::tb_action_ul_t* action, srsran_uci_data_t* uci_data)
{
  ...
  // Store last mcs_idx
  if (action && action->tb.enabled) {
    phy->stack->set_last_mcs(ue_ul_cfg.ul_cfg.pusch.grant.tb.mcs_idx);
  }
...
void cc_worker::set_uci_periodic_cqi(srsran_uci_data_t* uci_data)
{
  ...
  // Store the reported CQI
  if (uci_data->cfg.cqi.data_enable) {
    switch (uci_data->cfg.cqi.type) {
      case SRSRAN_CQI_TYPE_SUBBAND_UE:
        phy->stack->set_last_cqi(SRSRAN_CQI_TYPE_SUBBAND_UE, uci_data->value.cqi.subband_ue.subband_cqi);
        break;
      case SRSRAN_CQI_TYPE_WIDEBAND:
        phy->stack->set_last_cqi(SRSRAN_CQI_TYPE_WIDEBAND, uci_data->value.cqi.wideband.wideband_cqi);
      default:
        break;
    }
  }

...
static int parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("pcap.gw_filename", bpo::value<string>(&args->stack.pkt_trace.gw_pcap.filename)->default_value("/tmp/ue_gw.pcap"), "GW layer capture filename")

    ("pdcp.concatenating_timer_ms", bpo::value<uint32_t>(&args->stack.pdcp.concatenating_timer_ms)->default_value(5), "PDCP concatenating timer duration in milliseconds")

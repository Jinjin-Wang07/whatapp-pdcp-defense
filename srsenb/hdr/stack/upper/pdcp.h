...
public:
  ...
  virtual ~pdcp() {}
  void
  init(rlc_interface_pdcp* rlc_, rrc_interface_pdcp* rrc_, gtpu_interface_pdcp* gtpu_, srsran::timer_handler* timers_);
  void stop();

  void start_encrypted_pcap(srsran::pdcp_pcap* encrypted_pcap);
  void start_plaintext_pcap(srsran::pdcp_pcap* plaintext_pcap);
  void start_gw_pcap(srsran::gw_pcap* gw_pcap);
  ...
  void set_last_cqi(uint16_t rnti, srsran_cqi_type_t cqi_type, uint8_t cqi);
  void set_last_mcs(uint16_t rnti, uint32_t mcs_idx);

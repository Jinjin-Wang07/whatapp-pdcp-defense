...
public:
  ...
  void set_last_cqi(uint16_t rnti, srsran_cqi_type_t cqi_type, uint8_t cqi) { pdcp.set_last_cqi(rnti, cqi_type, cqi); };
  void set_last_mcs(uint16_t rnti, uint32_t mcs_idx) { pdcp.set_last_mcs(rnti, mcs_idx); };
...
private:
  ...
  srsran::pdcp_pcap    pdcp_encrypted_pcap = srsran::pdcp_pcap(PDCP_ENCRYPTED_DLT);
  srsran::pdcp_pcap    pdcp_plaintext_pcap = srsran::pdcp_pcap(PDCP_PLAIN_DLT);

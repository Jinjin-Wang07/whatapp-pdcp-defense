...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("pcap.pdcp_encrypted_enable",   bpo::value<bool>(&args->stack.pdcp_encrypted_pcap.enable)->default_value(false),         "Enable encrypted PDCP packet captures for wireshark")
    ("pcap.pdcp_encrypted_filename", bpo::value<string>(&args->stack.pdcp_encrypted_pcap.filename)->default_value("pdcp_encrypted.pcap"), "Encrypted PDCP layer capture filename")
    ("pcap.pdcp_plaintext_enable",   bpo::value<bool>(&args->stack.pdcp_plaintext_pcap.enable)->default_value(false),         "Enable plaintext PDCP packet captures for wireshark")
    ("pcap.pdcp_plaintext_filename", bpo::value<string>(&args->stack.pdcp_plaintext_pcap.filename)->default_value("pdcp_plaintext.pcap"), "Plaintext PDCP layer capture filename")
    ("pcap.gw_enable",   bpo::value<bool>(&args->stack.gw_pcap.enable)->default_value(false),         "Enable GW packet captures for wireshark")
    ("pcap.gw_filename", bpo::value<string>(&args->stack.gw_pcap.filename)->default_value("gw.pcap"), "GW packet capture filename")
    
    ("pdcp.concatenating_timer_ms",  bpo::value<uint32_t>(&args->stack.pdcp.concatenating_timer_ms)->default_value(5), "PDCP concatenating timer in milliseconds")

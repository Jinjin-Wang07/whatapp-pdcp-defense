                        ...
                        x2_interface*            x2_)
{
  ...
  // Set global PDCP configuration
  srsran::set_pdcp_global_config(args.pdcp.concatenating_timer_ms);
  ...
  if (args.pdcp_encrypted_pcap.enable) {
    pdcp_encrypted_pcap.open(args.pdcp_encrypted_pcap.filename.c_str());
    pdcp.start_encrypted_pcap(&pdcp_encrypted_pcap);
  }

  if (args.pdcp_plaintext_pcap.enable) {
    pdcp_plaintext_pcap.open(args.pdcp_plaintext_pcap.filename.c_str());
    pdcp.start_plaintext_pcap(&pdcp_plaintext_pcap);
  }

  if (args.gw_pcap.enable) {
    gw_pcap.open(args.gw_pcap.filename.c_str());
    pdcp.start_gw_pcap(&gw_pcap);
  }
  // add sync queue
  ...
  pdcp.init(&rlc, &rrc, gtpu_adapter.get(), task_sched.get_timer_handler());
...
void enb_stack_lte::stop_impl()
{
  ...
  if (args.pdcp_encrypted_pcap.enable) {
    pdcp_encrypted_pcap.close();
  }

  if (args.pdcp_plaintext_pcap.enable) {
    pdcp_plaintext_pcap.close();
  }

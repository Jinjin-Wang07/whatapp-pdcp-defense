...
void pdcp::start_encrypted_pcap(srsran::pdcp_pcap* pcap)
{
  encrypted_pcap = pcap;
}

void pdcp::start_plaintext_pcap(srsran::pdcp_pcap* pcap)
{
  plaintext_pcap = pcap;
}

void pdcp::start_gw_pcap(srsran::gw_pcap* pcap)
{
  gw_pcap = pcap;
}
...
void pdcp::add_user(uint16_t rnti)
{
  if (users.count(rnti) == 0) {
    ...
    obj->init(&users[rnti].rlc_itf, &users[rnti].rrc_itf, &users[rnti].gtpu_itf, timers);
    obj->start_encrypted_pcap(encrypted_pcap);
    obj->start_plaintext_pcap(plaintext_pcap);
    obj->start_gw_pcap(gw_pcap);
...
void pdcp::set_last_cqi(uint16_t rnti, srsran_cqi_type_t cqi_type, uint8_t cqi)
{
  if (users.count(rnti) && users[rnti].pdcp != nullptr) {
    users[rnti].pdcp->set_last_cqi(cqi_type, cqi);
  }
};
void pdcp::set_last_mcs(uint16_t rnti, uint32_t mcs_idx)
{
  if (users.count(rnti) && users[rnti].pdcp != nullptr) {
    users[rnti].pdcp->set_last_mcs(mcs_idx);
  }
};
} // namespace srsenb

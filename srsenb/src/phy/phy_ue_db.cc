                              ...
                              stack_interface_phy_lte*       stack)
{
  ...
  switch (cqi_cfg.type) {
    case SRSRAN_CQI_TYPE_WIDEBAND:
      ...
      stack->set_last_cqi(rnti, SRSRAN_CQI_TYPE_WIDEBAND, stack_value);
      break;
    case SRSRAN_CQI_TYPE_SUBBAND_UE:
      ...
      stack->sb_cqi_info(tti,
                         ...
                         stack_value);
      stack->set_last_cqi(rnti, SRSRAN_CQI_TYPE_SUBBAND_UE, stack_value);
      break;
    case SRSRAN_CQI_TYPE_SUBBAND_HL:
      stack_value = cqi_value.subband_hl.wideband_cqi_cw0;
      // Todo: change interface
      stack->cqi_info(tti, rnti, cqi_cc_idx, stack_value);
      stack->set_last_cqi(rnti, SRSRAN_CQI_TYPE_SUBBAND_HL, stack_value);
      break;
    case SRSRAN_CQI_TYPE_SUBBAND_UE_DIFF:
      ...
      stack->set_last_cqi(rnti, SRSRAN_CQI_TYPE_SUBBAND_UE_DIFF, stack_value);
      break;
  }
}
                             ...
                             const srsran_uci_value_t& uci_value)
{
  ...
  if (uci_value.cqi.data_crc) {
    ...
    if (uci_cfg.cqi.data_enable) {
      send_cqi_data(
          tti, rnti, cqi_cc_idx, uci_cfg.cqi, uci_value.cqi, ue.cell_info[0].phy_cfg.dl_cfg.cqi_report, cell, stack);
    }
